...
module.exports = class Log {
    ...
    static warn(message, err) {
        message = this.stringify(message);
        console.log( '[' + moment().format("YYYY-MM-DD HH:mm:ss") + ']', ' Warn '.bgYellow.white, message, err || '');
    }
    
    static error(message, err) {
        message = this.stringify(message);
        console.log( '[' + moment().format("YYYY-MM-DD HH:mm:ss") + ']', ' Error '.bgRed.white, message, err || '');
    }
